 ...
 *   5. Focus trap — confine Tab cycling within the open dialog
 *
 * The CSS-only mechanism remains fully functional without this script.
 * All enhancements here provide keyboard and focus behavior that CSS alone
 * cannot provide.
 */
(function () {
    'use strict';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        /**
         * Return the first currently open lightbox checkbox.
         */
        function getOpenToggle() {
            return document.querySelector('.lightbox-toggle:checked');
        }

        /**
         * Return the configured gallery target id for an open toggle.
         */
        function getGalleryTargetId(toggle, direction) {
            var overlay = getOverlayForCheckbox(toggle);
            if (!overlay) {
                return null;
            }

            var selector = direction === 'previous'
                ? '.lightbox-gallery-prev'
                : '.lightbox-gallery-next';
            var control = overlay.querySelector(selector);
            return control ? control.getAttribute('data-lightbox-target') : null;
        }

        /**
         * Close the current lightbox and open another by checkbox id.
         */
        function openLightboxById(targetId) {
            var target = document.getElementById(targetId);
            if (!target || !target.classList.contains('lightbox-toggle')) {
                return;
            }
         ...
         * 3. Arrow keys — navigate an open gallery
         * ============================================================== */
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape') {
                ...
                var direction = e.key === 'ArrowLeft' ? 'previous' : 'next';
                var targetId = getGalleryTargetId(openToggle, direction);
                if (targetId) {
                    e.preventDefault();
                    openLightboxById(targetId);
                }
            }
        });

        var galleryControls = document.querySelectorAll('.lightbox-gallery-control');
        ...
        /* ==============================================================
         * 4. Focus management — on open / close
         *
        ...
        /* ==============================================================
         * 5. Focus trap — confine Tab within the open overlay
         *
